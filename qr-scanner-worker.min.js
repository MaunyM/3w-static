...
(function () {
    ...
    function T(a, b) {
        ...
        for (; 4 <= a.available();) {
            ...
            if (d === y.ECI) 0 === a.readBits(1) ? c.chunks.push({
                ...
                }) : c.chunks.push({type: u.ECI, assignmentNumber: -1}); else if (d === y.Numeric) {
                var e = a;
                d = [];
                for (var g = "", f = e.readBits([10, 12, 14][b]); 3 <= f;) {
                    var h = e.readBits(10);
                    if (1E3 <= h) throw Error("Invalid numeric value above 999");
                    var k = Math.floor(h / 100), m = Math.floor(h / 10) % 10;
                    h %= 10;
                ...
                if (2 === f) {
                    f = e.readBits(7);
                    if (100 <=
                        f) throw Error("Invalid numeric value above 99");
                    e = Math.floor(f / 10);
                    f %= 10;
                    d.push(48 + e, 48 + f);
                ...
                d = [];
                g = "";
                for (f = e.readBits([9, 11, 13][b]); 2 <= f;) m = e.readBits(11), k = Math.floor(m / 45), m %= 45, d.push(B[k].charCodeAt(0), B[m].charCodeAt(0)),
                    g += B[k] + B[m], f -= 2;
            ...
            } else if (d === y.Byte) d = S(a, b), c.text += d.text, c.bytes.push(...d.bytes), c.chunks.push({
                type: u.Byte,
                bytes: d.bytes,
                text: d.text
            }); else if (d === y.Kanji) {
                ...
                for (f = 0; f < e; f++) k = g.readBits(13), k = Math.floor(k / 192) << 8 | k % 192, k = 7936 > k ? k + 33088 : k + 49472, d.push(k >> 8, k & 255);
    ...
    function W(a, b) {
        ...
        c.set(a);
        a = new X(285, 256, 0);
        var d = new w(a, c), e = new Uint8ClampedArray(b), g = !1;
        for (var f = 0; f < b; f++) {
        ...
        if (1 === f) b = [b.getCoefficient(1)]; else {
            e = Array(f);
            g = 0;
            for (h = 1; h < a.size && g < f; h++) 0 === b.evaluateAt(h) && (e[g] = a.inverse(h), g++);
            b = g !== f ? null : e
        }
        ...
        for (f = 0; f < e; f++) {
            h = a.inverse(b[f]);
            let c = 1;
            for (let d = 0; d < e; d++) f !== d && (c = a.multiply(c, J(1, a.multiply(b[d], h))));
    ...
    function Y(a, b, c) {
        c = Z[c.dataMask];
        let d = a.height;
        var e = 17 + 4 * b.versionNumber, g = A.createEmpty(e, e);
        g.setRegion(0, 0, 9, 9, !0);
        g.setRegion(e - 8, 0, 8, 9, !0);
        g.setRegion(0, e - 8, 9, 8, !0);
        for (var f of b.alignmentPatternCenters) for (var h of b.alignmentPatternCenters) 6 === f && 6 === h || 6 === f && h === e - 7 || f === e - 7 && 6 === h || g.setRegion(f - 2, h - 2, 5, 5, !0);
    ...
    function da(a, b, c) {
        let d = b.errorCorrectionLevels[c], e = [], g = 0;
        ...
        if (1 < d.ecBlocks.length) for (f = d.ecBlocks[0].numBlocks, b = d.ecBlocks[1].numBlocks, c =
            0; c < b; c++) e[f + c].codewords.push(a.shift());
    ...
    function fa(a, b) {
        var c = ea({x: 3.5, y: 3.5}, {x: b.dimension - 3.5, y: 3.5}, {
                x: b.dimension - 6.5,
                y: b.dimension - 6.5
            }, {x: 3.5, y: b.dimension - 3.5}), d = M(b.topLeft, b.topRight, b.alignmentPattern, b.bottomLeft),
            e = d.a11 * c.a11 + d.a21 * c.a12 + d.a31 * c.a13, g = d.a12 * c.a11 + d.a22 * c.a12 + d.a32 * c.a13,
            f = d.a13 * c.a11 + d.a23 * c.a12 + d.a33 * c.a13, h = d.a11 * c.a21 + d.a21 * c.a22 + d.a31 *
            c.a23, k = d.a12 * c.a21 + d.a22 * c.a22 + d.a32 * c.a23, m = d.a13 * c.a21 + d.a23 * c.a22 + d.a33 * c.a23,
            p = d.a11 * c.a31 + d.a21 * c.a32 + d.a31 * c.a33, n = d.a12 * c.a31 + d.a22 * c.a32 + d.a32 * c.a33,
    ...
    function ha(a, b, c) {
        let d = x(a, b), e = x(b, c), g = x(a, c), f, h, k;
        e >= d && e >= g ? [f, h, k] = [b, a, c] : g >= e && g >= d ? [f, h, k] = [a, b, c] : [f, h, k] = [a, c, b];
        0 > (k.x - h.x) * (f.y - h.y) - (k.y - h.y) * (f.x - h.x) && ([f, k] = [k, f]);
    ...
    function N(a, b, c, d) {
        ...
        let k = Math.abs(a - f), m = Math.abs(b - h), p = Math.floor(-k / 2), n = f < a ? 1 : -1, l = h < b ? 1 : -1,
            q = !0;
        for (let r = f, F = h; r !== a + n; r += n) {
            f = g ? F : r;
            h = g ? r : F;
            if (c.get(f, h) !== q && (q = !q, e.push({x: f, y: h}), e.length === d + 1)) break;
            p += m;
            if (0 < p) {
                if (F === b) break;
                F += l;
        ...
        c =
            [];
        for (g = 0; g < d; g++) e[g] && e[g + 1] ? c.push(x(e[g], e[g + 1])) : c.push(0);
        return c
    }

    function z(a, b, c, d) {
        let e = b.y - a.y, g = b.x - a.x;
    ...
    function O(a, b, c) {
        try {
            let d = z(a, {x: -1, y: a.y}, c, b.length), e = z(a, {x: a.x, y: -1}, c, b.length),
                g = z(a, {x: Math.max(0, a.x - a.y) - 1, y: Math.max(0, a.y - a.x) - 1}, c, b.length),
                f = z(a, {x: Math.min(c.width, a.x + a.y) + 1, y: Math.min(c.height, a.y + a.x) + 1}, c, b.length),
                h = G(d, b), k = G(e, b), m = G(g, b), p = G(f, b),
    ...
    function ja(a) {
            ...
            for (let b =
                ...
                if (g === m) k++; else {
                    ...
                    if (f) {
                        ...
                        h = c.filter((b) => d >= b.bottom.startX && d <= b.bottom.endX || a >= b.bottom.startX && d <= b.bottom.endX || d <= b.bottom.startX && a >= b.bottom.endX && 1.5 > l[2] / (b.bottom.endX -
                            b.bottom.startX) && .5 < l[2] / (b.bottom.endX - b.bottom.startX));
                        0 < h.length ? h[0].bottom = f : c.push({top: f, bottom: f})
                    }
                    if (g) {
                        let a = b - l[4], c = a - l[3];
                        g = {startX: c, y: r, endX: a};
                        f = e.filter((b) => c >= b.bottom.startX && c <= b.bottom.endX || a >= b.bottom.startX && c <= b.bottom.endX || c <= b.bottom.startX && a >= b.bottom.endX && 1.5 > l[2] / (b.bottom.endX - b.bottom.startX) && .5 < l[2] / (b.bottom.endX - b.bottom.startX));
        ...
        b = b.filter((a) => 2 <= a.bottom.y - a.top.y).map((b) => {
            ...
            if (a.get(Math.round(c), Math.round(d))) return b = [b.top.endX - b.top.startX, b.bottom.endX - b.bottom.startX, b.bottom.y - b.top.y + 1], b = t(b) / b.length, {
                score: O({
                    x: Math.round(c),
                    y: Math.round(d)
                }, [1, 1, 3, 1, 1], a), x: c, y: d, size: b
            }
        }).filter((a) =>
            !!a).sort((a, b) => a.score - b.score).map((a, b, c) => {
            if (4 < b) return null;
        ...
        var {topRight: k, topLeft: m, bottomLeft: p} = ha(b[0].points[0], b[0].points[1], b[0].points[2]);
        let n;
        try {
            ({
                dimension: n,
        ...
        d = d.map((b) => {
            ...
            if (a.get(Math.floor(c), Math.floor(d))) return t([b.top.endX - b.top.startX, b.bottom.endX - b.bottom.startX, b.bottom.y - b.top.y + 1]), b = O({
                x: Math.floor(c),
                y: Math.floor(d)
            }, [1, 1, 1], a) + x({x: c, y: d}, q), {x: c, y: d, score: b}
        }).filter((a) => !!a).sort((a, b) => a.score -
            b.score);
    ...
    function I(a, b, c, d = {}) {
        ...
        var f = e.greyScaleWeights, h = e.canOverwriteImage, k = b * c;
        ...
        p = new R(b, c, p);
        if (f.useIntegerApproximation) for (var n = 0; n < c; n++) for (var l = 0; l < b; l++) {
            var q = 4 * (n * b + l);
            p.set(l, n, f.red * a[q] + f.green * a[q + 1] + f.blue * a[q + 2] + 128 >> 8)
        } else for (n = 0; n < c; n++) for (l = 0; l < b; l++) q = 4 * (n * b + l), p.set(l, n, f.red * a[q] + f.green * a[q +
        1] + f.blue * a[q + 2]);
        f = Math.ceil(b / 8);
        n = Math.ceil(c / 8);
        l = f * n;
        ...
        r = new R(f, n, r);
        for (l = 0; l < n; l++) for (q = 0; q < f; q++) {
            var u = 0, v = Infinity, t = 0;
            for (let a = 0; 8 > a; a++) for (let b = 0; 8 > b; b++) {
        ...
        q = null;
        g && (h ? (a =
            new Uint8ClampedArray(a.buffer, m, k), q = new A(a, b)) : q = A.createEmpty(b, c));
        for (b = 0; b < n; b++) for (a = 0; a < f; a++) {
            c = f - 3;
            c = 2 > a ? 2 : a > c ? c : a;
            h = n - 3;
            h = 2 > b ? 2 : b > h ? h : b;
            k = 0;
            for (m = -2; 2 >= m; m++) for (v = -2; 2 >= v; v++) k += r.get(c + m, h + v);
            c = k / 25;
            for (h = 0; 8 > h; h++) for (k = 0; 8 > k; k++) m = 8 * a + h, v = 8 * b + k, t = p.get(m, v), l.set(m, v, t <= c), g && q.set(m, v, !(t <= c))
        }
    ...
    class A {
        ...
        setRegion(a, b, c, d, e) {
            for (let g = b; g < b + d; g++) for (let b = a; b < a + c; b++) this.set(b, g, !!e)
        }
    }
    ...
    class U {
        ...
        readBits(a) {
            ...
            if (0 < a) {
                ...
                0 < a && (c = 8 - a, b = b << a | (this.bytes[this.byteOffset] &
    ...
    class w {
        ...
        multiplyPoly(a) {
            if (this.isZero() ||
                a.isZero()) return this.field.zero;
            ...
            for (let h = 0; h < c; h++) {
                let c = b[h];
                for (let b = 0; b < d; b++) {
                    var g = h + b, f = this.field.multiply(c, a[b]);
                    e[g] = e[h + b] ^ f
                }
            }
    ...
    class X {
        constructor(a, b, c) {
            ...
            a = 1;
            for (b = 0; b < this.size; b++) this.expTable[b] = a, a *= 2, a >= this.size && (a = (a ^ this.primitive) &
                this.size - 1);
    ...
    let K = [{
            ...
            }], ca = [{bits: 21522, formatInfo: {errorCorrectionLevel: 1, dataMask: 0}}, {
            bits: 20773,
        ...
        Z = [(a) => 0 === (a.y + a.x) % 2, (a) => 0 === a.y % 2, (a) => 0 === a.x % 3, (a) => 0 === (a.y + a.x) % 3, (a) => 0 === (Math.floor(a.y / 2) + Math.floor(a.x / 3)) % 2, (a) => 0 === a.x * a.y % 2 + a.x * a.y % 3, (a) => 0 === (a.y * a.x % 2 + a.y * a.x % 3) % 2, (a) => 0 === ((a.y + a.x) % 2 + a.y * a.x % 3) % 2],
        x = (a, b) => Math.sqrt(Math.pow(b.x - a.x, 2) + Math.pow(b.y - a.y, 2)), ka = {
            inversionAttempts: "attemptBoth",
            greyScaleWeights: {red: .2126, green: .7152, blue: .0722, useIntegerApproximation: !1},
            canOverwriteImage: !0
        };
    I.default = I;
    let H = "dontInvert", D = {red: 77, green: 150, blue: 29, useIntegerApproximation: !0};
    self.onmessage = (a) => {
        let b = a.data.data;
        switch (a.data.type) {
            case "decode":
                a = I(b.data, b.width, b.height, {inversionAttempts: H, greyScaleWeights: D});
